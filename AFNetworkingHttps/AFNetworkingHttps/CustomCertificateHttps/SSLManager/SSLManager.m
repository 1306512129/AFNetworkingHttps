    ...
    if (!isTest) {
        ...
        [manager setSessionDidReceiveAuthenticationChallengeBlock:^NSURLSessionAuthChallengeDisposition(NSURLSession *session, NSURLAuthenticationChallenge *challenge, NSURLCredential *__autoreleasing *_credential) {
            ...
            if([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
                ...
                NSString *p12 = [[NSBundle mainBundle] pathForResource:isTest?@"Test":@"Real"ofType:@"p12"];
...
+ (AFSecurityPolicy*)customSecurityPolicyWithRequestType:(NSString *)requestType {
    ...
    if ([requestType isEqualToString:@"51"]) {
        cerPath = [[NSBundle mainBundle] pathForResource:isTest?@"Test":@"Real" ofType:@"cer"];//证书的路径
    }else {
        cerPath = [[NSBundle mainBundle] pathForResource:isTest?@"Test":@"Real" ofType:@"cer"];//证书的路径
    }
...
+ (BOOL)extractIdentity:(SecIdentityRef*)outIdentity andTrust:(SecTrustRef *)outTrust fromPKCS12Data:(NSData *)inPKCS12Data {
    ...
    NSDictionary*optionsDictionary = [NSDictionary dictionaryWithObject:isTest?caTest_Password:caPassword
